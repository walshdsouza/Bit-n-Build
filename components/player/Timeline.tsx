...
export default function Timeline({ playing, onPlayPause }: TimelineProps) {
  ...
  return (
    <div className="bg-surface-container-lowest border-t border-outline-variant/30 px-6 py-3 flex-shrink-0">
      ...
      <div className="relative mb-2 flex items-center gap-3">
        <span className="text-xs font-mono text-outline w-10 text-right shrink-0">0:08</span>
        <div className="flex-1 relative h-1.5 bg-surface-container rounded-full overflow-hidden cursor-pointer">
          {/* Buffered */}
          <div className="absolute inset-y-0 left-0 w-[55%] bg-primary/20 rounded-full" />
          {/* Played */}
          <div
            className="absolute inset-y-0 left-0 bg-gradient-to-r from-secondary-container to-primary rounded-full"
            style={{ width: `${progress}%` }}
          />
        </div>
        <span className="text-xs font-mono text-outline w-10 shrink-0">0:28</span>
        {/* Draggable thumb */}
        <input
          type="range"
          min={0} max={100}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 ml-2">
          ...
          <div className="w-20 relative h-1.5 bg-surface-container rounded-full overflow-hidden">
            <div className="absolute inset-y-0 left-0 bg-primary/70 rounded-full" style={{ width: `${volume}%` }} />
