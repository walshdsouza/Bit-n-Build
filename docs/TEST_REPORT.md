...
Website deployment `dpl_25sSN3vWqybTD9SDaWZJEE9vdAhT` is live at `https://unmute-ai.vercel.app`, built from commit `320ddab`. All 20 focused production browser checks passed in 2.1 minutes, including landing/branding, Profile-only Settings, larger player, blue progress, mute controls, native MP4 playback, responsive layout and resumable imports. These browser cases use controlled video/provider responses; actual provider evidence is described separately above.

The production API also passed a completed-continuation integration probe containing the genuinely translated 826 captions, preserving every timestamp and translation; this did not repeat a fresh YouTube import. Tampered and wrong-video tokens were rejected. A real synthetic five-second audio fixture passed `/api/live` through Groq and generated 32 signs. Evidence: `logs/production-api-release-320ddab.json`.

Twelve extension checks passed: five host/actual iframe cases, four runtime/ownership/security cases, two real-recorder regressions and one compiled-widget speech-to-signing case. The final case served the actual widget bundle and NEXA model, mocked only runtime/storage/provider responses, and measured 26 GPU-uploaded poses with substantive forearm, hand and finger movement. It also proved pre-Start/forged messages make no provider requests and Stop aborts transcription and ignores stale speech. TypeScript and scoped ESLint passed. Mozilla lint reports zero errors and notices, with six bundled React `innerHTML` warnings. The unsigned Firefox ZIP is under `extension/web-ext-artifacts/`; actual Firefox/Meet validation and the feature gaps against deployed `/live` remain in `extension/AgentFixThis.md`.
